...
<img  class="divider" src="image/divider.png" height="20" width="1600">
		...
		<?php
			...
			foreach(${'test_pv_'.$loop_arr[$i]} as $label => $value)
		...
		<div id="chartContainer",style="height: 300px; width: 100%;">
			...
			foreach ($pv_2016 as $value) {

			$new_pv_2016[] = $value/end($pv_2016);//change year
			};
			array_pop($new_pv_2016);
		
			//print_r ($new_pv_2016);
			
			$test = array_combine($manager, $new_pv_2016);
			//print_r($test)."<br/>";
			$test1 =array();//-change associate array name
			foreach($test as $label => $value)
				{array_push($test1, array("y"=>$value,"label"=>$label));};
...
//window.onload = function () {
	...
	{
		title:{
			text: " 2016 Allocation of Fossoil Fuel Exposure between Managers"
		},
		...
		{       
			...
			indexLabel: "{label} {y}",
			dataPoints: 
			<?php echo json_encode($test1_pv_2014, JSON_NUMERIC_CHECK); ?>
